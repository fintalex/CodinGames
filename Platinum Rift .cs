...
class PlatinumRift
{
    ...
    private static int StepToZone(Zone zoneFrom, Zone zoneTo)
    {
        ...
        if (stepedZones.Count > 0)
        {
            ...
            Zone lastZone = stepedZones.Find(z => z.ID == zoneTo.ID);
    ...
    {
        foreach (Zone zone in allZones.FindAll(z => z.Step == nextStep - 1))
        {
    ...
    private static string GetRowForMove()
    {
        ...
        foreach (var myZone in allMyZonesPODs)
        {
            ...
            else
            {
                ...
                Console.Error.WriteLine("STEP: " + enemyZone.ID + " Iam: " + myZone.ID + " stepToEnemy: " + stepToEnemy);
