...
class PlatinumRift
{
    ...
    private static string GetRowForBuy()
    {
        ...
        {
            // change strategy
            orderedZone = listZone.FindAll(zx => zx.Owner == -1 && (zx.Plbars >= 4)).OrderBy(z => z.Plbars).ToList();
            if (orderedZone.Count > 0)
            {
                for (int i = 0; i < orderedZone.Count; i++)
                {
                    ...
                    if (orderedZone[i].Neighbors.Count > 2)
                        buy += "1 " + orderedZone[i].Neighbors[2].ID + " ";
                }
            }
        }
        else
        ...
        if (playerCount>2)
        {
            List<Zone> Ark = listZone.FindAll(j => 
                ( j.ID == 57 ||  j.ID == 67 || j.ID == 78 || j.ID == 89 || j.ID == 97 || j.ID == 104 || j.ID == 113 ) 
                ).OrderByDescending(z=>z.Plbars).ToList();

            if (Ark.Count > 0 && Ark[0].Owner == -1)
            {
                buy = "2 " + Ark[0].ID + " " + buy;
            }
        }
