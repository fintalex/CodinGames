...
class PlatinumRift
{
	...
	private static string GetRowForMove()
	{
		...
		{
			//// here I have to move my PODs to free zone (with much count of PLAbars)
			//List<Zone> freeZone = myZone.Neighbors.FindAll(n => n.Owner != myId).OrderByDescending(n=>n.Plbars).ToList();
			//if (freeZone.Count > 0)
			//{
			//	move += myZone.Players.Find(p => p.ID == myId).PODs + " " + myZone.ID + " " + freeZone[0].ID + " ";
			//}
			//else
			//{
